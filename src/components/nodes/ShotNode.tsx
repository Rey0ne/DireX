          ...
          {phase === 'overview' && getOverview() && (
            <div style={{ display:'flex',flexDirection:'column',gap:6 }}>
              ...
              {getOverview().scenes && getOverview().scenes.length>0 && <>
                ...
                <div key={i} onClick={()=>{const ov=getOverview();if(!ov)return;const sc=ov.scenes[i];if(!sc)return;const shots=(ov.allShots||[]).find((s:any)=>s.sceneNumber===sc.sceneNumber)?.shots;if(!shots?.length)return;const store=useCanvasStore.getState();const BX=(store.nodes.get(id)?.pos?.x||0)+340;const BY=(store.nodes.get(id)?.pos?.y||0)+200;shots.forEach((sh:any,si:number)=>{const nid=store.addNode('image.generate',{x:BX+(si%4)*340,y:BY+Math.floor(si/4)*400},sh.shotType+' #'+(sc.sceneNumber||'')+'-'+sh.shotNumber);store.updateNode(nid,{meta:{gen:{prompt:sh.visualPrompt,videoPrompt:sh.videoPrompt||'',model:'GPT Image2',aspect:'16:9',resolution:'2K',quality:'high'},characters:sc.characters||[],sceneType:sc.sceneType||'',shot:{shotType:sh.shotType,cameraMovement:sh.cameraMovement,angle:sh.angle,aperture:sh.aperture}}});});}} style={{
