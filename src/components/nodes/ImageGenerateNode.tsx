  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      ...
      <div style={{ position: 'relative' }}>
        ...
        <div
        ref={cardRef}
        onMouseEnter={() => { if (data.imageUrl) setImgHover(true); }}
        ...
        onClick={() => { if (data.imageUrl) setShowFullscreen(true); }}
      >
        ...
        <div style={{
          ...
          {data.imageUrl ? (
            <>
              ...
              {/* Hover overlay: fullscreen + download */}
              {imgHover && (
                <div style={{
                  ...
                  <OverlayBtn icon="⛶" label="全屏" onClick={(e) => { e.stopPropagation(); setShowFullscreen(true); }} />
                  <OverlayBtn icon="↓" label="下载" onClick={(e) => { e.stopPropagation(); handleDownload(); }} />
                </div>
              )}
            </>
          ) : (
      ...
      {selected && !data.multiSelect && cardRect && createPortal(
        <div style={{
          position: 'fixed',
          left: cardRect.left + (cardRect.width - 760) / 2,
...
function OverlayBtn({ icon, label, onClick }: { icon: string; label: string; onClick: (e: React.MouseEvent) => void }) {
  return (
    <button
      onClick={onClick}
      title={label}
