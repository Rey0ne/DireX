...
interface ImageGenNodeData {
  ...
  onGridExpand?: () => void;
  onGridCollapse?: () => void;
  // Shot metadata (from script analysis)
  ...
  return (
    ...
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      ...
      <div style={{ position: 'relative' }}>
        ...
        {/* Shot metadata tag — shown when node has script analysis shot data */}
        {data.shot && (data.shot.shotType || data.shot.shotFunction) && (
          <div style={{
            position: 'absolute', top: '-20px', right: '4px', zIndex: 10,
            display: 'flex', alignItems: 'center', gap: 4,
          }}>
            {data.shot.shotType && (
              <span style={{ fontSize: 9, fontWeight: 700, color: '#00CFFF', letterSpacing: '0.05em' }}>
                {data.shot.shotType}
              </span>
            )}
            {data.shot.shotFunction && (
              <span style={{ fontSize: 8, color: 'var(--tap-text-4)', letterSpacing: '0.03em' }}>
                {data.shot.shotFunction}
              </span>
            )}
          </div>
