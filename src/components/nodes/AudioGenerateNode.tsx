...
import { getAudioDirexCost } from '../../store/pricing';
      ...
      {selected && !data.multiSelect && (
        ...
        <div style={{ background: '#fff', borderRadius: 'var(--tap-r-xl)', pointerEvents: 'auto', boxShadow: 'inset 0 0 0 1px rgba(0,207,255,0.06), inset 0 0 10px rgba(0,207,255,0.03), 0 0 0 3px rgba(0,207,255,0.04), 0 0 0 8px rgba(0,207,255,0.02), 0 2px 12px rgba(0,0,0,0.03)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '8px 12px', borderTop: '1px solid var(--tap-divider)', flexWrap: 'wrap' }}>
            ...
            <div style={{ flex: 1 }} />
            <div style={{display:'flex',alignItems:'center',justifyContent:'flex-end',height:'20px',borderRadius:'10px',background:'linear-gradient(135deg,rgba(0,0,0,0.03) 0%,rgba(0,0,0,0.01) 50%,rgba(0,0,0,0.03) 100%)',border:'1px solid var(--tap-divider)',boxShadow:'0 0 10px rgba(0,0,0,0.02),inset 0 1px 0 rgba(0,0,0,0.03)',flexShrink:0,paddingLeft:'8px',paddingRight:'2px',gap:'6px'}}>
              {getAudioDirexCost(currentModel) > 0 && !isElevenLabs && (
                <span style={{fontSize:'9px',color:'#000',fontWeight:500,whiteSpace:'nowrap',fontFamily:'Inter, sans-serif'}}>{getAudioDirexCost(currentModel)}</span>
              )}
