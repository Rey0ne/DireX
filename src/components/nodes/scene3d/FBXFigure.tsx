...
import { extractSkeleton } from './Skeleton/SkinnedFigure';
import { BoneController } from './Skeleton/BoneController';
...
/** Renders a cached FBX group (memoized + wrapped) + skeleton overlay */
export function FBXFigure({ src }: { src: string }) {
  const group = useFBXModel(src);
  const { wrapped, skeleton } = useMemo(() => {
    if (!group) return { wrapped: null, skeleton: null };
    const g = new THREE.Group();
    const clone = group.clone(true);
    g.add(clone);
    return { wrapped: g, skeleton: extractSkeleton(clone) };
  }, [group]);
  if (!wrapped) return null;
  return (
    <group>
      <primitive object={wrapped} />
      {skeleton && <BoneController skeleton={skeleton} selectedBone={null} onSelectBone={() => {}} gizmoRef={{ current: null }} />}
    </group>
  );
}
