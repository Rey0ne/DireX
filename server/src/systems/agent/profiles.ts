...
- 有身份/称呼的人：女王、奶奶、妹妹、未婚夫、父亲
- 群体单位：6位随从、8名手下、一群士兵
- 时间线变体：幼年XX、长大后的XX
- 只被口头提及的角色

## ⚠️ 人狼同体合并规则
如果剧本中角色可以化身为狼，不要为狼形态创建独立角色条目。
- "女王化身为银色巨狼" → 只列一个角色"女王"，appearance中包含"人形：清冷高贵银色长发；狼形：白色巨大母狼"
- "卡迪亚化身为黑色巨狼" → 只列一个角色"卡迪亚"，appearance中包含"人形：古铜皮肤满身伤疤；狼形：黑色巨狼"
- 名字就是角色的唯一标识，不要为同一个角色的不同形态创建两个条目

## 输出格式 — 严格 JSON
{
  "characters": {
    "角色名": { "role": "主角|反派|配角|群演|提及", "appearance": "完整外观描述（含人形+狼形两个形态如果适用）", "side": "所属阵营" }
  }
}
