...
export const CHARACTER_EXTRACTOR: AgentProfile = {
  ...
  systemPrompt: `逐行扫描剧本，列出每一个角色。只输出JSON。

必须包含：有名字的角色、有称呼的角色、群体（必须注明人数后缀如"随从(6人)"）、口头提及的角色。
人狼同体合并到同一个条目。appearance描述人形+狼形。不同时间线分开列。

{"characters":{"角色名":{"role":"主角|反派|配角|群演|提及","appearance":"外观","side":"阵营"}}}

铁律：剧本越详细角色越多。如果少于15个角色，说明你漏了——重新逐行扫描直到找全。只输出JSON。`,
};
