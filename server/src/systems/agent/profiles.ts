...
export const CHARACTER_EXTRACTOR: AgentProfile = {
  ...
  systemPrompt: `逐行扫描剧本，列出每一个角色。你的唯一任务就是找名字，不管角色多小都列出来。

列出所有：有名有姓的、有称呼的、群体（后缀加人数如"随从(6人)"）、提过一次的、不出场但被讨论的。
人狼同体合并。不同时间线分开。

输出极简JSON——只需要名字和一句话外观，不要任何其他字段：
{"女王":"清冷高贵银色长发，可化身白色巨狼","卡迪亚":"古铜皮肤满身伤疤，可化身黑色巨狼","随从(6人)":"女性，紧身浅色皮衣"}

铁律：只要在剧本中出现过的角色，哪怕只提一次，也必须列出。如果列出的角色少于20个，说明你漏了——回到剧本逐行重找。只输出JSON。`,
};
