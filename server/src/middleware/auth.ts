...
export function authMiddleware(req: Request, res: Response, next: NextFunction) {
  ...
  if (!req.path.startsWith('/api/') || req.path === '/api/health' || req.path === '/api/download' || req.path === '/api/proxy-image' || req.path === '/api/last-compiled' || req.path.startsWith('/admin') || req.path === '/api/kie-callback' || req.path.startsWith('/api/auth')) return next();
